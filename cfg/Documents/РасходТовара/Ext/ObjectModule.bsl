...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	// Создадим запрос, чтобы получать информацию о составе документа
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТоварыВДокументе.Ссылка.Дата КАК Период,
	|	ТоварыВДокументе.Товар КАК Товар,
	|	ВЫБОР
	|		КОГДА ТоварыВДокументе.Товар.Вид = ЗНАЧЕНИЕ(Перечисление.ВидыТоваров.Товар)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоТовар,
	|	ТоварыВДокументе.Ссылка.Склад КАК Склад,
	|	ТоварыВДокументе.Ссылка.Покупатель КАК Покупатель,
	|	ТоварыВДокументе.Количество * ТоварыВДокументе.ЕдиницаИзмерения.Коэффициент КАК Количество,
	|	ТоварыВДокументе.Сумма КАК Сумма
	|ИЗ
